...
func testUsers(t *testing.T, s store.Users) {
	...
	//	req.NoError(s.TruncateUsers(ctx))
	//	u := makeNew()
	//	req.NoError(tmp.(storeInterface).ManagedTx(ctx, func(jobs chan<- bulk.Job) error {
	//		jobs <- bulk.CreateUser(u)
	//
	//		lookup := bulk.LookupUserByID(u.ID)
	//		jobs <- lookup
	//		collectedUser, err := lookup.Collect()
	//		req.NoError(err)
	//		req.NotNil(collectedUser)
	//		req.Equal(u.Email, collectedUser.Email)
	//		return nil
	//	}))
	//})
